...
const P = {
  manny_id: {
    type: "string",
    description:
      "Full string ID of the Manny (e.g. mny_e84fa37181de693e8e831147)",
  },
  object_id: { type: "string", description: "Sector object ID" },
  container_id: {
    type: "string",
    description: "Inventory item ID of the storage container",
  },
...
export const TOOLS: ChatCompletionTool[] = [
      ...
      properties: {
        ...
        recipe: {
          ...
          enum: [
            ...
            "solar_panel",
            "scut_relay",
            "thermal_protection_shell",
            "parachute_pack",
            "descent_guidance_module",
            "atmospheric_drop_kit",
            "manny",
          ],
          description:
            "Recipe ID to craft. All Manny-craftable items. Note: scut_relay must be activated separately with turn_on_relay once deployed in sector.",
        },
      },
    },
  ),
    ...
    "Order a Manny to detach a storage container from the probe. Can leave it drifting freely or hide it on a specific asteroid.",
    {
      ...
      properties: {
        ...
        mode: {
          ...
          description:
            "drifting: leave floating in sector. hidden_on_asteroid: hide on a specific asteroid (requires asteroid_object_id).",
        },
        asteroid_object_id: {
          type: "string",
          description:
            "Required when mode=hidden_on_asteroid: sector object ID of the asteroid to hide the container on.",
        },
      },
    },
  ),
  ...
  defineTool("rename_manny", "Rename a Manny.", {
    ...
    "Deploy a Manny from probe inventory into active service, making it available to receive tasks. Use when the operator asks to 'activate', 'deploy', or 'add' a manny that is currently stowed in inventory. The item_id comes from the STOWED IN INVENTORY section of the mannies list in the current state.",
    {
      ...
      properties: {
        item_id: {
          type: "string",
          description:
            "Inventory item ID of the stowed manny (from the STOWED IN INVENTORY list)",
        },
      },
    },
  ),
  ...
  defineTool(
    ...
    "Discard resources or items from the probe's inventory.",
    {
      ...
      properties: {
        inventory_id: {
          type: "string",
          description: "Inventory item ID to jettison",
        },
        amount: {
          type: "number",
  ...
  defineTool(
    ...
    "Order a Manny to salvage an abandoned object in the current sector (e.g. abandoned Mannies, drifting containers).",
    {
      ...
      properties: {
        ...
        object_id: {
          type: "string",
          description: "Object ID in the current sector to salvage",
        },
      },
    },
  ),
    ...
    "Order a Manny to carry a storage container from the probe and hide it on a specific asteroid in the current sector. The container remains retrievable later with recover_container.",
    {
      ...
      properties: {
        ...
        object_id: {
          type: "string",
          description:
            "Sector object ID of the asteroid to hide the container on",
        },
      },
    },
  ),
    ...
    "Schedule an action to be executed automatically when a condition is met (e.g. 'move probe when manny-1 is idle'). The poller checks every 30 seconds. Use this instead of asking the user to re-issue the command later.",
    {
      ...
      properties: {
        description: {
          type: "string",
          description:
            "Human-readable description of what this scheduled action will do",
        },
        condition: {
          type: "object",
          ...
          properties: {
            type: {
              ...
              description:
                "manny_idle: wait for a specific manny to finish its task. probe_idle: wait for the probe to stop moving.",
            },
            mannyId: {
              type: "string",
              description: "Required when type=manny_idle: the manny's full ID",
            },
            mannyName: {
              type: "string",
              description:
                "Required when type=manny_idle: the manny's display name (for the log)",
            },
            requireItems: {
              type: "array",
              items: { type: "string" },
              description:
                "Optional (manny_idle only): also wait until ALL of these item types exist in probe inventory before firing. Use this for parallel build dependencies — e.g. schedule the final assembly step with requireItems=['electric_motor'] so it only fires once the motor (being built in parallel by another manny) is ready.",
            },
          },
        },
        action: {
          ...
          properties: {
            ...
            mannyId: {
              type: "string",
              description:
                "craft_item / mine_resources / detach_container / recover_container: manny to use",
            },
            recipe: { type: "string", description: "craft_item: recipe ID" },
            objectId: {
              type: "string",
              description:
                "mine_resources / recover_container: sector object ID",
            },
            resources: {
              type: "array",
              items: { type: "string" },
              description: "mine_resources: resource types",
            },
            targetAmount: {
              type: "number",
              description: "mine_resources: ECE amount",
            },
            targetContainerId: {
              type: "string",
              description: "mine_resources: optional container to deposit into",
            },
            containerId: {
              type: "string",
              description:
                "detach_container: inventory item ID of the container",
            },
          },
        },
      },
    ...
    "Order a Manny to activate an inactive SCUT relay in the current sector. Duration: ~5 minutes. Consumes one integrated_circuit from inventory. The sector must contain a star (solar power). Once active the relay creates or extends a SCUT network allowing long-range probe communication and remote Manny tasking.",
    {
      ...
      properties: {
        ...
        relay_id: {
          type: "number",
          description:
            "Integer ID of the inactive SCUT relay sector object. SCUT relay sector objects have purely numeric IDs (e.g. sector object id='42' → relay_id=42). Use get_game_state to see relays in sector.",
        },
        network_name: {
          type: "string",
          description:
            "Optional: name for a new SCUT network if this relay starts an isolated one. Ignored when relay joins existing coverage.",
        },
      },
    },
  ),
  ...
  defineTool(
    ...
    "Discard the cargo of a Manny that is currently waiting outside the probe for storage space, then retry docking. Resource cargo is permanently lost. Recoverable objects (salvaged Mannies, drifting items, detached containers) are restored to the sector as drifting objects. Use when a Manny is stuck waiting and you want to free it.",
    {
      ...
      properties: {
        manny_id: {
          type: "string",
          description: "Full string ID of the waiting Manny",
        },
      },
    },
  ),
    ...
    "Send a message to another probe or an inhabited planet. Probe recipients must be in the same sector or reachable via a shared SCUT relay network. Planet recipients must be inhabited planets in the current sector.",
    {
      ...
      properties: {
        ...
        recipient_id: {
          type: "string",
          description:
            "Probe numeric ID (e.g. '652') or planet sector object ID (e.g. 'planet-abc123')",
        },
        body: {
          type: "string",
    ...
    "Order a Manny to transfer deuterium from this probe's reserve to another probe or drone in the same sector. Duration: ~5 minutes. Use get_game_state to find other probes visible in the sector.",
    {
      ...
      properties: {
        ...
        target_probe_id: {
          type: "number",
          description:
            "Integer numeric ID of the target probe or drone (not its name — use the id field from the probe list in sector objects)",
        },
        amount: {
          type: "number",
    ...
    "Order a Manny to assemble a brand-new Von Neumann probe outside the current hull. The new probe can then be piloted via SCUT or the operator can transfer their instance into it (the current probe becomes a drone). Requires two distinct empty storage containers from inventory plus the appropriate components. The Manny must be idle.",
    {
      ...
      properties: {
        ...
        container_ids: {
          ...
          items: { type: "string" },
          description:
            "Exactly two distinct inventory item IDs of empty storage containers to use as the new probe's hull sections",
        },
      },
    },
  ),
    ...
    "Order a Manny to install a waypoint bookmark beacon on a sector object (asteroid, planet, or star). The beacon transmits a named message readable by any probe in the sector. Requires a waypoint_bookmark item in inventory.",
    {
      ...
      properties: {
        ...
        object_id: {
          type: "string",
          description:
            "Sector object ID of the asteroid, planet, or star to place the bookmark on",
        },
        name: {
          type: "string",
    ...
    "Order a Manny to drop a storage container down onto a planet surface using an atmospheric drop kit (ablative shield + parachute). Consumes one atmospheric_drop_kit from inventory. The container descends and lands on the planet.",
    {
      ...
      properties: {
        ...
        planet_id: {
          type: "string",
          description:
            "Sector object ID of the planet to drop the container onto",
        },
      },
    },
  ),
...
async function handleGetGameState(
  ...
  const activeMannyIds = new Set(
    (manniesResp.mannies ?? []).map((m: any) => m.id),
  );
  return {
    probe: {
      id: probe.id,
    ...
    probe_improvements: (improvementsResp?.improvements ?? []).map(
      (imp: any) => ({
        id: imp.id,
        name: imp.name,
...
function makeHandlers(probeId?: number | null): Record<string, Handler> {
  ...
  return {
    ...
    mine_resources: (a) =>
      c.mineResources(
        a.manny_id as string,
        a.object_id as string,
        a.resources as string[],
        a.target_amount as number,
        a.target_container_id as string | undefined,
      ),
    detach_container: (a) =>
      c.detachContainer(
        a.manny_id as string,
        a.container_id as string,
        (a.mode as "drifting" | "hidden_on_asteroid") ?? "drifting",
        a.asteroid_object_id as string | undefined,
      ),
    repair_manny: (a) =>
      c.repairManny(a.manny_id as string, a.integrity_percent as number),
    ...
    scan_sector: (a) =>
      client.scanSector(a.x as number, a.y as number, a.z as number),
    jettison_item: (a) =>
      c.jettisonItem(a.inventory_id as string, a.amount as number | undefined),
    salvage_object: (a) =>
      c.salvageObject(a.manny_id as string, a.object_id as string),
    inspect_sector_object: (a) =>
      c.inspectSectorObject(a.manny_id as string, a.object_id as string),
    inspect_asteroid: (a) =>
      c.inspectSectorObject(a.manny_id as string, a.object_id as string),
    recover_container: (a) =>
      c.recoverContainer(a.manny_id as string, a.object_id as string),
    drop_container_on_asteroid: (a) =>
      c.dropContainerOnAsteroid(
        a.manny_id as string,
        a.container_id as string,
        a.object_id as string,
      ),
    ...
    transfer_deuterium: (a) =>
      c.transferDeuteriumToProbe(
        a.manny_id as string,
        a.target_probe_id as number,
        a.amount as number,
      ),
    assemble_probe: (a) =>
      c.assembleProbe(a.manny_id as string, a.container_ids as string[]),
    improve_probe: (a) =>
      c.improveProbe(a.manny_id as string, a.improvement as string),
    install_waypoint_bookmark: (a) =>
      c.installWaypointBookmark(
        a.manny_id as string,
        a.object_id as string,
        a.name as string,
      ),
    drop_container_on_planet: (a) =>
      c.dropContainerOnPlanet(
        a.manny_id as string,
        a.container_id as string,
        a.planet_id as string,
      ),
    turn_on_relay: (a) =>
      c.turnOnRelay(
        a.manny_id as string,
        a.relay_id as number,
        a.network_name as string | undefined,
      ),
    drop_manny_cargo: (a) => c.dropMannyCargo(a.manny_id as string),
    send_message: (a) =>
      c.sendMessage(
        a.recipient_type as "probe" | "planet",
    ...
    schedule_action: async (a) => {
      const entry = await addPendingAction({
        ...
        // Scope the row to whatever probe this handler set was built for, so the
        // poller later runs it against that probe — not always the main one.
        probeId: probeId ?? null,
      });
      return {
        ok: true,
        scheduledActionId: entry.id,
        message: `Scheduled action #${entry.id}: "${entry.description}". The poller will execute it within 30 seconds of the condition being met.`,
      };
    },
    cancel_scheduled_action: async (a) => {
      ...
      return cancelled
        ...
        : {
            ok: false,
            message: `No pending scheduled action with ID ${a.id} found.`,
          };
    },
  };
}
